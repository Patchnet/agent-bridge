...
async function replyToEmail(messageId, htmlContent) {
  ...
  // Fetch original to validate the reply-to address
  const original = await graphRequest('GET', `/me/messages/${messageId}?$select=from,toRecipients,ccRecipients`);
  const replyTo = original.from?.emailAddress?.address;
  if (replyTo) validateRecipients([replyTo]);
